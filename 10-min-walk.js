...
function isValidWalk(walk) {
  //it will always be 10 for it to work
  if (walk.length != 10) {
    return false;
  }
  ...
  for (let i = 0; i < walk.length; i++) {
    //which letter of the walk, which direction
    const direction = walk[i];
    //then add that to the count
    count[direction]++;
  }
  // whatevr way you go you have to come the equal way back and
  //always end up with 0
  if (count.n - count.s == 0 && count.e - count.w == 0) {
    return true;
  } else return false;
}
