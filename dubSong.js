...
// in his performance. Recently, he has decided to take a couple of old songs and make dubstep remixes from them.
...
//  words "WUB" before the first word of the song (the number may be zero), after
//  the last word (the number may be zero), and between words (at least one between
...
function songDecoder(song) {
  //creat an empty array for new song to go into
  const oldSong = [];
  //split the song on the WUB's
  const splitSong = song.split("WUB");
  console.log("im the song", splitSong);
  //iterate over the split song
  for (i = 0; i < splitSong.length; i++) {
    if (splitSong[i] !== "") {
      oldSong.push(splitSong[i]);
    }
    console.log("im old", oldSong);
  }
  return oldSong.join(" ");
}
